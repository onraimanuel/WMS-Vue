    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title" style="color: red;">Laporan Stok Barang</h3>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-sm-9">
            ...
            <div class="col-sm-2">
              ...
              <SearchInput v-model="searchText" @search-input="handleSearchInput" placeholder="Search"
                class="form-control" />
            </div>
          </div>
          <table id="example1" class="table table-bordered table-striped" style="width: 95%">
            ...
            <tbody>
              <tr v-for="(item, index) in displayedProducts" :key="index">
                ...
                <td class="text-center">{{ item.stokMasuk }}</td>
                <td class="text-center">{{ item.stokKeluar }}</td>
            ...
            <div class="col-md-6">
              <nav>
                <ul class="pagination justify-content-start">
                  ...
                  <li v-for="n in pagesToShow" :key="n" :class="{ 'page-item': true, active: currentPage === n }">
                    <button class="page-link" @click="gotoPage(n)">
                      {{ n }}
                    </button>
                  </li>
...
export default {
  ...
  data() {
    return {
      dataStok: [
        { namaToko: 'Loesi komp', category: 'makanan', namaProduk: 'Tela-tela', stokMasuk: 10, stokKeluar: 1 },
        { namaToko: 'Pt. Tbk', category: 'makanan', namaProduk: 'Sasagun', stokMasuk: 10, stokKeluar: 1 },
        { namaToko: 'Koperasi IT Del', category: 'makanan', namaProduk: 'Piza', stokMasuk: 10, stokKeluar: 1 },
        { namaToko: 'Loesi komp', category: 'makanan', namaProduk: 'Tela-tela', stokMasuk: 10, stokKeluar: 1 },
        { namaToko: 'Pt. Tbk', category: 'makanan', namaProduk: 'Sasagun', stokMasuk: 10, stokKeluar: 1 },
        { namaToko: 'Koperasi IT Del', category: 'makanan', namaProduk: 'Piza', stokMasuk: 10, stokKeluar: 1 },
        { namaToko: 'Loesi komp', category: 'makanan', namaProduk: 'Tela-tela', stokMasuk: 10, stokKeluar: 1 },
  ...
  methods: {
    handleSearchInput(searchValue) {
      this.filteredData = this.dataStok.filter(item =>
        item.namaToko.toLowerCase().includes(searchValue.toLowerCase()) ||
        item.category.toLowerCase().includes(searchValue.toLowerCase()) ||
        item.namaProduk.toLowerCase().includes(searchValue.toLowerCase()) ||
        item.stokMasuk.toString().toLowerCase().includes(searchValue.toLowerCase()) ||
        item.stokKeluar.toString().toLowerCase().includes(searchValue.toLowerCase())
      );
    },
    prevPage() {
